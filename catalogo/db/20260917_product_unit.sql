BEGIN;
ALTER TABLE public.pl_catalog_products ADD COLUMN IF NOT EXISTS unit_name text;
ALTER TABLE public.pl_catalog_products ADD CONSTRAINT pl_catalog_products_unit_length CHECK (char_length(unit_name) <= 100);
...
BEGIN
  ...
  IF p_action IN ('stage','duplicate') THEN
    ...
    IF p_action='duplicate' THEN
      ...
      INSERT INTO public.pl_catalog_products(store_id,name,description,price,original_price,parcelas,installment_interest_rate,
        categoria,categoria_vitrine,categorias_vitrine,is_active,is_featured,unit_name,client_request_id)
      VALUES(p_store_id,source_row.name||' (cópia)',source_row.description,source_row.price,source_row.original_price,
        ...
        source_row.categorias_vitrine,false,source_row.is_featured,source_row.unit_name,p_data->>'request_id') RETURNING id INTO pid;
  ...
  IF p_action='save' THEN
    ...
    UPDATE public.pl_catalog_products SET name=coalesce(nullif(trim(p_data->>'name'),''),'Produto sem nome'),
      ...
      unit_name=CASE WHEN p_data ? 'unit_name' THEN nullif(trim(p_data->>'unit_name'),'') ELSE unit_name END,
