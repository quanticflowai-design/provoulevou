-- Chave criada apenas no banco; o gerador recebe seu hash.
do $$
declare
  v_slug constant text := 'oticapires';
  v_email constant text := 'oticapires30@gmail.com';
  v_phone constant text := '5564992480395';
  v_domain constant text := 'https://provoulevou.com.br/catalogo/?loja=oticapires';
  v_key text;
  v_lead uuid;
begin
  ...
  values (v_slug,'Ótica Pires','https://provoulevou.com.br/catalogo/assets/logo-otica-pires.webp',v_phone,v_email,
    'basic',true,'#087A7E',
    '{"bg":"#F4FBFB","card":"#FFFFFF","line":"#C6E9E8","brand":"#087A7E","dark":"#075B60","soft":"#E2F4F3","on":"#FFFFFF","cta":"#B50D70","ctaDark":"#880853","onCta":"#FFFFFF"}'::jsonb,
    3,v_key);
  ...
  values ('Ótica Pires',v_domain,v_email,'Ótica Pires',v_phone,'basic','Teste Gratuito',true,
  ...
  select id into v_lead from public.leads
    where lower(coalesce(email,''))=v_email
      or regexp_replace(coalesce(telefone,''),'[^0-9]','','g') in ('64992480395',v_phone)
      or instagram='catalogo:oticapires'
    order by created_at desc limit 1;
  if v_lead is null then
    insert into public.leads
      (instagram,nome_loja,telefone,whatsapp,email,status,fonte_oportunidade,plataforma,teste_gratis_em,notas)
    values ('catalogo:oticapires','Ótica Pires',v_phone,v_phone,v_email,'testando','catalogo','catalogo',current_date,
  ...
  insert into public.interacoes(lead_id,tipo,conteudo)
    select v_lead,'nota','Catálogo Ótica Pires criado; teste grátis de 7 dias iniciado.'
    where not exists (select 1 from public.interacoes where lead_id=v_lead and tipo='nota'
      and conteudo='Catálogo Ótica Pires criado; teste grátis de 7 dias iniciado.');
end $$;
