-- Óptica Santa Efigênia: catálogo, autenticação do gerador e teste grátis no CRM.
...
select
  'santaefigenia', 'Óptica Santa Efigênia',
  'https://cdn.jsdelivr.net/gh/quanticflowai-design/provoulevou@56f80c3/catalogo/assets/logo-santaefigenia.webp',
  '5535998392035', 'Óptica Santa Efigênia', '#F15A24',
  'pl_cat_' || encode(gen_random_bytes(32), 'hex'),
  'cidapimentaa66@gmail.com', 'basic', true,
  '{"bg":"#fff8f3","card":"#ffffff","brand":"#F15A24","cta":"#C74316","onCta":"#ffffff"}'::jsonb,
  3
where not exists (
  select 1 from public.pl_catalog_stores
   where slug = 'santaefigenia' or lower(owner_email) = 'cidapimentaa66@gmail.com'
);
...
select
  'whatsapp_5535998392035', 'Óptica Santa Efigênia', 0, false, 'testando',
  'Registrado pelo deploy do Provou Catálogo. Catálogo criado para teste grátis de 7 dias.',
  'pt', true, 'WhatsApp', '5535998392035', 'cidapimentaa66@gmail.com',
  '5535998392035', 'oculos', 'BR', 'instagram'
where not exists (
  select 1 from public.leads
   where instagram = 'whatsapp_5535998392035'
      or regexp_replace(coalesce(telefone, ''), '\D', '', 'g') in ('35998392035', '5535998392035')
      or regexp_replace(coalesce(whatsapp, ''), '\D', '', 'g') in ('35998392035', '5535998392035')
);
...
select id, 'nota',
       'Catálogo da Óptica Santa Efigênia criado e lead movido para o teste grátis de 7 dias.', now()
  from public.leads l
 where l.instagram = 'whatsapp_5535998392035'
   and not exists (
     select 1 from public.interacoes i
      where i.lead_id = l.id
        and i.conteudo = 'Catálogo da Óptica Santa Efigênia criado e lead movido para o teste grátis de 7 dias.'
   );
