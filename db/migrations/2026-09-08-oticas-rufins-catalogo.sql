-- Óticas Rufins: catálogo, autenticação do gerador e teste grátis no CRM.
...
select
  'oticasrufins', 'Óticas Rufins',
  'https://cdn.jsdelivr.net/gh/quanticflowai-design/provoulevou@437c261/catalogo/assets/logo-oticasrufins.webp',
  '5571997115400', 'Óticas Rufins', '#E0B900',
  'pl_cat_' || encode(gen_random_bytes(32), 'hex'),
  'rufians.site@gmail.com', 'basic', true,
  '{"bg":"#fffdf4","card":"#ffffff","brand":"#E0B900","cta":"#3F3F43","onCta":"#ffffff"}'::jsonb,
  3
where not exists (
  select 1 from public.pl_catalog_stores
   where slug = 'oticasrufins' or lower(owner_email) = 'rufians.site@gmail.com'
);
...
select
  'whatsapp_5571997115400', 'Óticas Rufins', 0, false, 'testando',
  'Registrado pelo deploy do Provou Catálogo. Catálogo criado para teste grátis de 7 dias.',
  'pt', true, 'WhatsApp', '5571997115400', 'rufians.site@gmail.com',
  '5571997115400', 'oculos', 'BR', 'instagram'
where not exists (
  select 1 from public.leads
   where instagram = 'whatsapp_5571997115400'
      or regexp_replace(coalesce(telefone, ''), '\D', '', 'g') in ('71997115400', '5571997115400')
      or regexp_replace(coalesce(whatsapp, ''), '\D', '', 'g') in ('71997115400', '5571997115400')
);

update public.leads set status = 'testando', updated_at = now()
 where instagram = 'whatsapp_5571997115400';
...
select id, 'nota',
       'Catálogo da Óticas Rufins criado e lead movido para o teste grátis de 7 dias.', now()
  from public.leads l
 where l.instagram = 'whatsapp_5571997115400'
   and not exists (
     select 1 from public.interacoes i where i.lead_id = l.id
       and i.conteudo = 'Catálogo da Óticas Rufins criado e lead movido para o teste grátis de 7 dias.'
   );
